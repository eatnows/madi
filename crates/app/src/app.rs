...
pub struct Tab {
    ...
    pub preview: bool,
    pub editor: Editor,
    /// Markdown files can show a rendered preview instead of raw/highlighted text.
...
impl Madi {
    ...
    fn is_markdown_file(path: &Path) -> bool {
        path.extension().and_then(|e| e.to_str()).is_some_and(|ext| ext.eq_ignore_ascii_case("md") || ext.eq_ignore_ascii_case("markdown"))
    }

    fn toggle_markdown_preview(&mut self) {
        let ws = self.workspaces.get_mut(&self.repo);
        let Some(tab) = ws.and_then(|ws| ws.active.and_then(|i| ws.tabs.get_mut(i))) else { return };
        if Self::is_markdown_file(&tab.path) {
    ...
    fn open_tab(&mut self, path: PathBuf, preview: bool) {
        ...
        let tab = Tab { path: path.clone(), title: name, preview, editor: Editor::new(&text, path, language), markdown_preview: false };
        let ws = self.workspace_mut();
        ws.showing_diff = false;
    ...
    fn markdown_preview_view(&self, cx: &mut Cx, p: &Palette) -> El {
        let Some(tab) = self.active_tab() else { return div().grow() };
        let id = ("markdown-preview", &tab.path);
        let width = cx.viewport(id).map_or(600., |v| v.0) - 32.;
        div()
            .id(id)
            .grow()
            .overflow_y_scroll()
            .bg(p.bg)
            .scrollbar(p.text_dim.with_alpha(0.4))
            .child(markdown::render(cx, p, &tab.editor.doc.text(), width))
    }

    fn main_area(&self, cx: &mut Cx, p: &Palette) -> El {
        let showing_diff = self.workspace().is_some_and(|ws| ws.showing_diff);
        let markdown_preview = !showing_diff && self.active_tab().is_some_and(|t| t.markdown_preview);
        let content = if showing_diff {
            self.diff_view(cx, p)
        } else if markdown_preview {
            self.markdown_preview_view(cx, p)
        ...
        let find_bar = if showing_diff || markdown_preview { None } else { self.find_bar(p) };
    ...
    fn event(&mut self, event: &Event, cx: &mut Cx) {
        match event {
            ...
            Event::KeyDown { key, text, .. } => {
                ...
                if matches!(key, gyeol::Key::Char(c) if c.eq_ignore_ascii_case("v")) && cx.modifiers.command() && cx.modifiers.shift {
...
mod tests {
    ...
    #[test]
    fn markdown_files_can_toggle_a_rendered_preview() {
        let (mut host, _, path) = app("md-preview");
        let readme = Path::new(&path).join("README.md");
        std::fs::write(&readme, "# Title\n\nSome **bold** text.\n").unwrap();
        host.state_mut().open_file(readme, false);
        host.frame();
    ...
    fn shortcut_shift(host: &mut TestHost<Madi>, letter: &str) {
        host.set_modifiers(Modifiers { shift: true, ..cmd() });
        host.key(Key::Char(letter.into()), Some(letter));
        host.set_modifiers(Modifiers::default());
    }
